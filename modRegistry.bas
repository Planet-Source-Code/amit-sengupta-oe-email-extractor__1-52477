...
Function SetDWORDValue(ByVal SubKey As String, ByVal Entry As String, ByVal Value As Long)
...
End Function
Function GetDWORDValue(ByVal SubKey As String, ByVal Entry As String)
...
Function SetBinaryValue(ByVal SubKey As String, ByVal Entry As String, ByVal Value As String)
    Dim i
    Call ParseKey(SubKey, MainKeyHandle)
...
Function GetBinaryValue(ByVal SubKey As String, ByVal Entry As String)
...
End Function
Function DeleteValue(ByVal KeyName As String, ByVal ValueName As String)
    ...
    If MainKeyHandle Then
       rtn = RegOpenKeyEx(MainKeyHandle, KeyName, 0, KEY_WRITE, hKey) 'open the key
       If rtn = ERROR_SUCCESS Then 'if the key could be opened then
          rtn = RegDeleteValue(hKey, ValueName) 'delete the value
          rtn = RegCloseKey(hKey)  'close the key
       End If
    End If
...
Function GetMainKeyHandle(ByVal MainKeyName As String) As Long
...
Function ErrorMsg(ByVal lErrorCode As Long) As String
    Dim GetErrorMsg
...
Function GetStringValue(ByVal SubKey As String, ByVal Entry As String)
...
Private Sub ParseKey(ByRef KeyName As String, ByRef Keyhandle As Long)
...
End Function
Function SetStringValue(ByVal SubKey As String, ByVal Entry As String, Value As String)
...
Public Function EnumRegistryKeys(ByVal SubKey As String) As Collection
    ...
    'SysLog 4, INFOLINE, "EnumRegistryKeys is searching in " & SubKey
    Call ParseKey(SubKey, MainKeyHandle)
    ...
    Do
       ...
       ' get the N-th key, exit the loop if not found
        If RegEnumKeyEx(hKey, index, subkeyName, length, 0, vbNullString, vbNull, fFiletime) = ERROR_NO_MORE_ITEMS Then Exit Do
        ...
        'SysLog 4, INFOLINE, "EnumRegistryKeys found " & subkeyName
    ...
    End If
End Function
Function EnumRegistryValues(ByVal SubKey As String) As Collection
    Dim index As Long
    ...
    Call ParseKey(SubKey, MainKeyHandle)
    If MainKeyHandle Then
        If RegOpenKeyEx(MainKeyHandle, SubKey, 0, KEY_READ, hKey) Then Exit Function
            ...
            Case REG_BINARY
                ...
                End If
                 'Convert to display as string like this: 00 01 01 00 01
                    resString = vbNullString
                    For i = 0 To UBound(resBinary) - 1
                         resString = resString & Chr(resBinary(i))
                    Next i
                    valueInfo(1) = resString 'Get rid of leading space
            Case REG_EXPAND_SZ
                If length <> 0 Then
                    resString = Space$(length - 1)
                    CopyMemory ByVal resString, resBinary(0), length - 1
